...
type FileHandle struct {
	...
	mu              sync.Mutex
	mpuId           *storage.MultipartBlobCommitInput
...
func (fh *FileHandle) mpuPartNoSpawn(buf *MBuf, part uint32, total int64, last bool) (err error) {
	...
	mpu := storage.MultipartBlobAddInput{
		Commit:     fh.mpuId,
		PartNumber: part,
		Body:       buf,
		Size:       uint64(buf.Len()),
...
func (fh *FileHandle) waitForCreateMPU() error {
	if fh.mpuId == nil {
		fh.mu.Unlock()
		fh.initWrite()
...
func (fh *FileHandle) readAhead(offset uint64, needAtLeast int) (err error) {
	...
	readAheadAmount := MaxReadHead

	for readAheadAmount-existingReadahead >= ReadHeadChunk {
		off := offset + uint64(existingReadahead)
		remaining := fh.inode.Attributes.Size - off

		// only read up to readahead chunk each time
		size := MinUInt32(readAheadAmount-existingReadahead, ReadHeadChunk)
		...
		if size != 0 {
			...
			readAheadBuf := ReadBuffer{}.Init(fh, off, size)
			if readAheadBuf != nil {
...
func (fh *FileHandle) readFile(offset int64, buf []byte) (bytesRead int, err error) {
	...
	if fh.seqReadAmount >= uint64(ReadHeadChunk) && fh.numOOORead < 3 {
		if fh.reader != nil {
...
func (fh *FileHandle) readFromStream(offset int64, buf []byte) (bytesRead int, err error) {
	...
	if fh.reader == nil {
		resp, err := fh.cloud.GetBlob(&storage.GetBlobInput{
			Key:   fh.key,
			Start: uint64(offset),
		})
		if err != nil {
...
func (fh *FileHandle) flushSmallFile() (err error) {
	...
	resp, err := fh.cloud.PutBlob(&storage.PutBlobInput{
		Key:         key,
		Body:        buf,
...
func (fh *FileHandle) updateFromFlush(etag *string, lastModified *time.Time, storageClass *string) {
	...
	if etag != nil {
		inode.sysMetadata["etag"] = []byte(*etag)
	}
	if storageClass != nil {
		inode.sysMetadata["storage-class"] = []byte(*storageClass)
	}
	if fh.keepPageCache {
...
func (b *ReadBuffer) initBuffer(fh *FileHandle, offset uint64, size uint32) {
	getFunc := func() (io.ReadCloser, error) {
		resp, err := b.cloud.GetBlob(&storage.GetBlobInput{
			Key:   fh.key,
			Start: offset,
			Count: uint64(size),
		})
