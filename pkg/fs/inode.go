...
func NewInode(fs *FileSystem, parent *Inode, name *string) (inode *Inode) {
	...
	return &Inode{
		...
		AttrTime:    time.Now(),
		Parent:      parent,
		sysMetadata: make(map[string][]byte),
		refcnt:      1,
	}
}

func deepCopyBlobItemOputput(item *storage.BlobItemOutput) storage.BlobItemOutput {
	key := NilStr(item.Key)
...
func (inode *Inode) SetFromBlobItem(item *storage.BlobItemOutput) {
	...
	if item.ETag != nil {
		inode.sysMetadata["etag"] = []byte(*itemcopy.ETag)
		inode.knownETag = itemcopy.ETag
	} else {
		delete(inode.sysMetadata, "etag")
	}
	if item.StorageClass != nil {
		inode.sysMetadata["storage-class"] = []byte(*itemcopy.StorageClass)
	} else {
		delete(inode.sysMetadata, "storage-class")
	}
	now := time.Now()
...
func (inode *Inode) cloud() (cloud storage.ObjectBackend, path string) {
	var prefix string
	var dir *Inode
...
func (inode *Inode) fillXattrFromHead(resp *storage.HeadBlobOutput) {
	...
	if resp.ETag != nil {
		inode.sysMetadata["etag"] = []byte(*resp.ETag)
	}
	if resp.StorageClass != nil {
		inode.sysMetadata["storage-class"] = []byte(*resp.StorageClass)
	} else {
		inode.sysMetadata["storage-class"] = []byte("STANDARD")
	}
...
func convertMetadata(meta map[string][]byte) (metadata map[string]*string) {
	...
	for k, v := range meta {
		k = strings.ToLower(k)
		metadata[k] = stringRef(xattrEscape(v))
	}
	return
}
...
func (inode *Inode) updateXattr() (err error) {
	...
	_, err = cloud.CopyBlob(&storage.CopyBlobInput{
		...
		Destination: key,
		Size:        &inode.Attributes.Size,
		ETag:        stringRef(string(inode.sysMetadata["etag"])),
...
func (inode *Inode) ListXattr() ([]string, error) {
	...
	for k, _ := range inode.sysMetadata {
