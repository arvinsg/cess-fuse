...
var bufferLog = utils.GetLogger("buffer")
...
func maxMemToUse(buffersNow uint64) uint64 {
	...
	availableMem, err := utils.GetCGroupAvailableMem()
	if err != nil {
	...
	maxBuffers := MaxUInt64(max/BuffSize, 1)
	log.Debugf("using up to %v %vMB buffers, now is %v", maxBuffers, BuffSize/1024/1024, buffersNow)
	return maxBuffers
}
...
func NewBufferPool(maxSizeGlobal uint64) *BufferPool {
	pool := BufferPool{maxBuffers: maxSizeGlobal / BuffSize}.Init()
	return pool
}

func (pool *BufferPool) RequestBuffer() (buf []byte) {
	return pool.RequestMultiple(BuffSize, true)[0]
}
			...
			if pool.numBuffers == 0 {
				...
				if pool.numBuffers+uint64(nPages) > pool.computedMaxbuffers {
					...
					log.Errorf("Unable to allocate %d bytes, limit is %d bytes",
						nPages*BuffSize, pool.computedMaxbuffers*BuffSize)
					panic("OOM")
				}
			}
			pool.cond.Wait()
